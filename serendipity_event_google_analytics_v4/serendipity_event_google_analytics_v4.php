...
class serendipity_event_google_analytics_v4 extends serendipity_event
{
    var $title = PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_NAME;
    ...
    function introspect(&$propbag)
    {
        ...
        $propbag->add('name', PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_NAME);
        $propbag->add('description', PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_DESC);
        $propbag->add('stackable', false);
        $propbag->add('author', 'Jari Turkia');
    ...
    function introspect_config_item($name, &$propbag)
    {
        switch ($name) {
            case 'analytics_account_number' :
                ...
                $propbag->add('name', PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_STREAM_NUMBER);
                $propbag->add('description', PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_STREAM_NUMBER_DESC);
            ...
            case 'analytics_track_external' :
                ...
                $propbag->add('name', PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_TRACK_EXTERNAL);
                $propbag->add('description', PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_TRACK_EXTERNAL_DESC);
                $propbag->add('default', 'true');
                break;

            default :
                $propbag->add('type', 'boolean');
                $propbag->add('name', sprintf(PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_APPLY_TRACKING_TO, constant($name)));
                $propbag->add('description', sprintf(PLUGIN_EVENT_GOOGLE_ANALYTICS_V4_APPLY_TRACKING_TO_DESC, constant($name)));
                $propbag->add('default', 'true');
        }
        return true;
    }
